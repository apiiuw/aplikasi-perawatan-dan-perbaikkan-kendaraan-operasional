...
<body class="flex justify-center items-center h-screen">
    <div class="flex w-full max-w-sm mx-auto overflow-hidden bg-white rounded-lg shadow-lg lg:max-w-6xl">
        ...
        <div class="w-full px-6 py-8 md:px-8 lg:w-1/2">
            ...
            <form method="POST" action="{{ route('login') }}">
                @csrf
                <div class="mt-4">
                    <label class="block mb-2 text-sm font-medium text-gray-600" for="login">NRP atau Email</label>
                    <input id="login" name="login" class="block w-full px-4 py-2 text-gray-700 bg-white border rounded-lg focus:border-blue-400 focus:ring-opacity-40 focus:outline-none focus:ring focus:ring-blue-300" type="text" value="{{ old('login') }}" required autofocus />
                    @error('login')
            ...
            <a href="{{ route('google.login') }}" class="flex items-center justify-center mt-4 text-gray-600 transition-colors duration-300 transform border rounded-lg hover:bg-gray-50">
                <div class="px-4 py-2">
